 ...
 *
 * FILES: notes.bin, tasks.bin, contacts.bin, events.bin, facts.bin, finances.bin, documents.bin
...
static int finances_rewrite(int skip_id, int set_deleted,
    const char *new_date, const char *new_desc, double new_amount,
    const char *new_cat, const char *new_acc, const char *new_notes);
static int documents_rewrite(int skip_id, int set_deleted,
    const char *new_title, const char *new_tmpl, const char *new_content);
...
static void data_path(char *buf, int size, const char *name) {
    size_t need = s_data_dir_set ? strlen(s_data_dir) + 1 + strlen(name) : strlen(name);
    if (need >= (size_t)(size - 1) || need >= (size_t)DATA_DIR_MAX)
        fprintf(stderr, "vibePDA: data path too long (data_dir=%zu, name=%zu)\n",
                (size_t)strlen(s_data_dir), (size_t)strlen(name));
    if (s_data_dir_set)
...
static void ensure_data_dir(void) {
    ...
    if (s_data_dir_set) {
        ...
        if (stat(s_data_dir, &st) != 0) {
            if (mkdir(s_data_dir, 0755) != 0)
                fprintf(stderr, "vibePDA: cannot create data dir %s: %s\n", s_data_dir, strerror(errno));
        }
    }
#endif
}
...
static void migrate_notes_txt_to_bin(const char *path_txt, const char *path_bin) {
    ...
    if (!in) { fprintf(stderr, "vibePDA: cannot open %s: %s\n", path_txt, strerror(errno)); return; }
    FILE *out = fopen(path_bin, "wb");
    if (!out) { fprintf(stderr, "vibePDA: cannot create %s: %s\n", path_bin, strerror(errno)); fclose(in); return; }
    char line[LINE_MAX];
...
typedef struct { int want_id; VibeNote *out; int found; } NoteFindCtx;
static void note_find_cb(const VibeNote *n, void *ctx) {
    NoteFindCtx *c = (NoteFindCtx *)ctx;
    if (n->id == c->want_id && c->out) { *c->out = *n; c->found = 1; }
}
int storage_note_get(int id, VibeNote *out) {
    NoteFindCtx ctx = { id, out, 0 };
    storage_notes_list(note_find_cb, &ctx);
    return ctx.found;
}
...
typedef struct { int want_id; VibeTask *out; int found; } TaskFindCtx;
static void task_find_cb(const VibeTask *t, void *ctx) {
    TaskFindCtx *c = (TaskFindCtx *)ctx;
    if (t->id == c->want_id && c->out) { *c->out = *t; c->found = 1; }
}
int storage_task_get(int id, VibeTask *out) {
    TaskFindCtx ctx = { id, out, 0 };
    storage_tasks_list(task_find_cb, &ctx);
    return ctx.found;
}
...
int storage_contacts_add(const char *name, const char *email, const char *phone, const char *notes) {
    ...
    char n[VIBE_NAME_MAX], e[VIBE_EMAIL_MAX], p[VIBE_PHONE_MAX], no[VIBE_CONTENT_MAX];
    copy_str(n, name ? name : "", VIBE_NAME_MAX);
    copy_str(e, email ? email : "", VIBE_EMAIL_MAX);
    copy_str(p, phone ? phone : "", VIBE_PHONE_MAX);
    copy_str(no, notes ? notes : "", VIBE_CONTENT_MAX);
    char ts[VIBE_DATETIME_MAX];
    timestamp(ts, sizeof(ts));
    FILE *f = fopen(path, "ab");
    if (!f) return 0;
...
typedef struct { int want_id; VibeContact *out; int found; } ContactFindCtx;
static void contact_find_cb(const VibeContact *c, void *ctx) {
    ContactFindCtx *fc = (ContactFindCtx *)ctx;
    if (c->id == fc->want_id && fc->out) { *fc->out = *c; fc->found = 1; }
}
int storage_contact_get(int id, VibeContact *out) {
    ContactFindCtx ctx = { id, out, 0 };
    storage_contacts_list(contact_find_cb, &ctx);
    return ctx.found;
}

static int contacts_rewrite(int skip_id, int set_deleted,
    const char *new_name, const char *new_email, const char *new_phone, const char *new_notes) {
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "contacts" EXT);
    char tmp[DATA_DIR_MAX + 72];
    snprintf(tmp, sizeof(tmp), "%s.$$$", path);
...
int storage_contacts_update(int id, const char *name, const char *email, const char *phone, const char *notes) {
    ...
    copy_str(p, phone ? phone : "", VIBE_PHONE_MAX);
    copy_str(no, notes ? notes : "", VIBE_CONTENT_MAX);
    return contacts_rewrite(id, 0, n, e, p, no);
}

int storage_contacts_delete(int id) {
    return contacts_rewrite(id, 1, NULL, NULL, NULL, NULL);
}
...
typedef struct { int want_id; VibeCalendarEvent *out; int found; } EventFindCtx;
static void event_find_cb(const VibeCalendarEvent *e, void *ctx) {
    EventFindCtx *c = (EventFindCtx *)ctx;
    if (e->id == c->want_id && c->out) { *c->out = *e; c->found = 1; }
}
int storage_event_get(int id, VibeCalendarEvent *out) {
    EventFindCtx ctx = { id, out, 0 };
    storage_events_list(event_find_cb, &ctx);
    return ctx.found;
}
...
typedef struct { int want_id; VibeFact *out; int found; } FactFindCtx;
static void fact_find_cb(const VibeFact *f, void *ctx) {
    FactFindCtx *c = (FactFindCtx *)ctx;
    if (f->id == c->want_id && c->out) { *c->out = *f; c->found = 1; }
}
int storage_fact_get(int id, VibeFact *out) {
    FactFindCtx ctx = { id, out, 0 };
    storage_facts_list(fact_find_cb, &ctx);
    return ctx.found;
}
...
int storage_trash_count(void) {
    return count_deleted_in_file("notes", notes_read_deleted) +
           ...
           count_deleted_in_file("finances", finances_read_deleted) +
           count_deleted_in_file("documents", documents_read_deleted);
}
...
static void trash_list_finances(void (*cb)(int, int, const char*, void*), void *ctx) {
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "finances" EXT);
    FILE *f = fopen(path, "rb");
    if (!f) return;
    while (1) {
        uint32_t id;
...
static void trash_list_documents(void (*cb)(int, int, const char*, void*), void *ctx) {
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "documents" EXT);
    FILE *f = fopen(path, "rb");
    if (!f) return;
    while (1) {
        uint32_t id;
...
void storage_trash_list(void (*cb)(int entity_type, int id, const char *title, void *), void *ctx) {
    ...
    trash_list_finances(cb, ctx);
    trash_list_documents(cb, ctx);
}
...
int storage_finances_add(const char *date, const char *description, double amount, const char *category, const char *account, const char *notes) {
    ...
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "finances" EXT);
    int id = next_id("finances");
    char d[VIBE_DATETIME_MAX], desc[VIBE_TITLE_MAX], cat[VIBE_TITLE_MAX], acc[VIBE_TITLE_MAX], n[VIBE_CONTENT_MAX];
    copy_str(d, date ? date : "", VIBE_DATETIME_MAX);
    copy_str(desc, description ? description : "", VIBE_TITLE_MAX);
    copy_str(cat, category ? category : "", VIBE_TITLE_MAX);
    copy_str(acc, account ? account : "", VIBE_TITLE_MAX);
    copy_str(n, notes ? notes : "", VIBE_CONTENT_MAX);
    char ts[VIBE_DATETIME_MAX];
    timestamp(ts, sizeof(ts));
    FILE *f = fopen(path, "ab");
    if (!f) return 0;
...
typedef struct { int want_id; VibeFinanceEntry *out; int found; } FinanceFindCtx;
static void finance_find_cb(const VibeFinanceEntry *fe, void *ctx) {
    FinanceFindCtx *c = (FinanceFindCtx *)ctx;
    if (fe->id == c->want_id && c->out) { *c->out = *fe; c->found = 1; }
}
int storage_finance_get(int id, VibeFinanceEntry *out) {
    FinanceFindCtx ctx = { id, out, 0 };
    storage_finances_list(finance_find_cb, &ctx);
    return ctx.found;
}
...
int storage_finances_update(int id, const char *date, const char *description, double amount, const char *category, const char *account, const char *notes) {
    ...
    copy_str(acc, account ? account : "", VIBE_TITLE_MAX);
    copy_str(n, notes ? notes : "", VIBE_CONTENT_MAX);
    return finances_rewrite(id, 0, d, desc, amount, cat, acc, n);
}

int storage_finances_delete(int id) {
    return finances_rewrite(id, 1, NULL, NULL, 0, NULL, NULL, NULL);
}
...
int storage_documents_add(const char *title, const char *template_name, const char *content) {
    ...
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "documents" EXT);
    int id = next_id("documents");
    char t[VIBE_TITLE_MAX], tn[VIBE_TITLE_MAX], c[VIBE_CONTENT_MAX];
    copy_str(t, title ? title : "", VIBE_TITLE_MAX);
    copy_str(tn, template_name ? template_name : "", VIBE_TITLE_MAX);
    copy_str(c, content ? content : "", VIBE_CONTENT_MAX);
    char ts[VIBE_DATETIME_MAX];
    timestamp(ts, sizeof(ts));
    FILE *f = fopen(path, "ab");
...
typedef struct { int want_id; VibeDocument *out; int found; } DocumentFindCtx;
static void document_find_cb(const VibeDocument *d, void *ctx) {
    DocumentFindCtx *c = (DocumentFindCtx *)ctx;
    if (d->id == c->want_id && c->out) { *c->out = *d; c->found = 1; }
}
int storage_document_get(int id, VibeDocument *out) {
    DocumentFindCtx ctx = { id, out, 0 };
    storage_documents_list(document_find_cb, &ctx);
    return ctx.found;
}
...
int storage_documents_update(int id, const char *title, const char *template_name, const char *content) {
    ...
    copy_str(tn, template_name ? template_name : "", VIBE_TITLE_MAX);
    copy_str(c, content ? content : "", VIBE_CONTENT_MAX);
    return documents_rewrite(id, 0, t, tn, c);
}

int storage_documents_delete(int id) {
    return documents_rewrite(id, 1, NULL, NULL, NULL);
}
...
int storage_empty_trash(void) {
    ...
    count += empty_trash_file("finances", finances_empty_record);
    count += empty_trash_file("documents", documents_empty_record);
    return count;
}
...
void storage_finances_list_filtered(void (*cb)(const VibeFinanceEntry *, void *), void *ctx, const char *query) {
    if (!query || !*query) { storage_finances_list(cb, ctx); return; }
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "finances" EXT);
    FILE *f = fopen(path, "rb");
    if (!f) return;
...
void storage_documents_list_filtered(void (*cb)(const VibeDocument *, void *), void *ctx, const char *query) {
    if (!query || !*query) { storage_documents_list(cb, ctx); return; }
    char path[DATA_DIR_MAX + 64];
    data_path(path, sizeof(path), "documents" EXT);
    FILE *f = fopen(path, "rb");
    if (!f) return;
